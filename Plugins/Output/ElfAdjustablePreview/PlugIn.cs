...
namespace ElfAdjustablePreview {
    ...
    public class PlugIn : IEventDrivenOutputPlugIn {
        ...
        private readonly List<VixenPlus.Channel> _channels;
        ...
        public PlugIn() {
            _channels = new List<VixenPlus.Channel>();
        }
        ...
        public void Initialize(IExecutable executableObject, SetupData setupData, XmlNode setupNode) {
            ...
            try {
                _profileFileName = ((Profile) executableObject).FileName ?? string.Empty;
            }
            catch {
                try {
                    _profileFileName = ((EventSequence) executableObject).Profile.FileName ?? string.Empty;
                    if (_profileFileName.Length == 0) {
                        _profileFileName = ((EventSequence) executableObject).Profile.Name + ".pro";
                    }
                }
        ...
        public void Startup() {
            ...
            var systemInterface = (ISystem) Interfaces.Available["ISystem"];
            var ci = typeof(PreviewDialog).GetConstructor(new[] { typeof(XmlNode), typeof(List<VixenPlus.Channel>), typeof(int) });
