...
namespace ElfAdjustablePreview.Profiles {
    ...
    public class BaseVixen : ElfDisposable, IProfile {
        ...
        protected List<ElfRawChannel> _channels;
        ...
        protected int MaxID {
            get {
                int MaxID = -1;
                foreach (ElfRawChannel Channel in Channels) {
        ...
        /// </summary>
        public BaseVixen() {
            Channels = new List<ElfRawChannel>();
        ...
        private ElfRawChannel GetChannel(int id) {
            foreach (ElfRawChannel item in Channels) {
                if (item.ID == id) {
                    return item;
                }
            }
        ...
        private bool LoadEPData(GeneralPlugIn data) {
            ...
            foreach (XmlNode ChannelNode in ChannelList) {
                ...
                Channel.RenderColor = _xmlHelper.GetNodeValue(ChannelNode, ElfChannel.Property_RenderColor, Color.Empty);
                Channel.BorderColor = _xmlHelper.GetNodeValue(ChannelNode, ElfChannel.Property_BorderColor, Color.Empty);

                Node = ChannelNode.SelectSingleNode(ElfChannel.Property_Origin);
                if (Node != null) {
        ...
        protected virtual ElfRawChannel LoadChannelFromXml(XmlNode node) {
            var Channel = new ElfRawChannel();
            ...
            Value = _xmlHelper.GetAttributeValue(node, ElfChannel.Property_Enabled.ToLower());
        ...
        private void SaveShuffles(XmlNode shuffleListNode) {
            ...
            foreach (string Shuffle in ShuffleList) {
                ...
                if (Name == Core.Core.Shuffle.NATIVE_SHUFFLE) {
                    continue;
                }
        ...
        /// </summary>
        protected virtual void SaveChannelName(XmlNode node, ElfRawChannel channel) {
        ...
        private void SaveEPData(XmlNode plugInNode) {
            ...
            foreach (ElfRawChannel Channel in Channels) {
                ...
                if (!Channel.Origin.IsEmpty) {
                    OriginNode = _xmlHelper.CreateNode(Doc, Node, ElfChannel.Property_Origin);
                ...
                if (!Channel.RenderColor.IsEmpty && !Channel.RenderColor.Equals(Channel.SequencerColor)) {
                    _xmlHelper.SetValue(Node, ElfChannel.Property_RenderColor, Channel.RenderColor);
                }

                if (!Channel.BorderColor.IsEmpty) {
                    _xmlHelper.SetValue(Node, ElfChannel.Property_BorderColor, Channel.BorderColor);
                }
        ...
        private bool Load(BaseProfile profile, XmlDocument doc, List<ElfRawChannel> rawChannels) {
            ElfRawChannel rawChannel = null;
            XmlNode Root = null;
            XmlNode Node = null;
            ...
            if ((rawChannels == null) || (rawChannels.Count == 0)) {
                ...
                foreach (ElfRawChannel rChannel in rawChannels) {
            ...
            int Idx = 0;
            foreach (ElfRawChannel rChannel in Channels) {
                Channel = new ElfChannel {
        ...
        /// </summary>
        public List<ElfRawChannel> Channels { get; set; }
        ...
        /// <returns>Returns true if the load is successful, false otherwise</returns>
        public virtual bool Load(BaseProfile profile, XmlNode setupNode, List<ElfRawChannel> rawChannels) {
            if (setupNode == null) {
