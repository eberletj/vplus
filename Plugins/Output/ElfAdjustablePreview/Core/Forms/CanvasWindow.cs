...
namespace ElfAdjustablePreview.Core.Forms
{
	public partial class CanvasWindow : ToolWindow
	{
		...
		private void CanvasPane_Paint(object sender, PaintEventArgs e)
		{
			...
			try
			{
				foreach (ElfChannel Channel in _profile.Channels)
				{
				...
				{
					foreach (ElfChannel SelectedChannel in _profile.Channels.Selected.OrderByDescending())
					{
				...
				{
					foreach (ElfChannel Channel in _workshop.ChannelMover.Channels)
					{
				...
				foreach (ElfImageStampChannel StampChannel in _workshop.StampChannels)
				{
		...
		private void UI_PropertyChanged(object sender, PropertyChangedEventArgs e)
		{
			switch (e.PropertyName)
			{
				case UISettings.Property_MousePosition:
					...
					foreach (ElfChannel Channel in _profile.Channels.Sorted)
					{
