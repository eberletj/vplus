...
namespace ElfAdjustablePreview.Core.Forms
{
	public partial class MappingPreview : Form
	{
		...
		public MappingPreview()
		{
			...
			_tempProfile.Channels.Add(new ElfChannel(TARGET));
			_tempProfile.Channels.Add(new ElfChannel(IMPORTED));
			_tempProfile.Channels.Add(new ElfChannel(ALREADY_IMPORTED));
		...
		private void DrawChannel(Graphics g, ElfChannel channel, Color brushColor, Color outlineColor, LatticePoint effectiveLatticeOffset)
		{
			if (!channel.Visible)
				return;
