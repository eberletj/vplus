...
using ElfAdjustablePreview.Core.XmlSerializers;
...
namespace ElfAdjustablePreview.Core.Channels
{
	...
	public class ElfChannel : ElfBase, ICloneable, INotifyPropertyChanged, IComparable, IItem
	{
		...
		public override string Serialized
		{
			get
			{
				if (_serialized.Length == 0)
					_serialized = Extends.SerializeObjectToXml<ElfChannel>(this);
		...
		[DebuggerHidden()]
		public ElfChannel(int index)
			: this()
		{
			Index = index;
		}

		public ElfChannel(ElfRawChannel rawChannel)
			: this()
		{
			base.SuppressEvents = true;
			_loading = true;
		...
		public override object Clone()
		{
			...
			((ElfChannel)MyClone).Lattice = new List<LatticePoint>();
			((ElfChannel)MyClone).Lattice.AddRange(Lattice);
			return MyClone;
		}
		...
		{
			if (obj is ElfChannel)
			{
				return ID.CompareTo((obj as ElfChannel).ID);
			}
			else
		...
		/// <exception cref="System.ArgumentNullException"></exception>
		public bool Differs(ElfChannel channel)
		{
			if (channel == null)
		...
		public string SerializeLattice(bool dedupeData, bool flatten)
		{
			...
			if (flatten)
			{
				foreach (ElfChannel SubChannel in _subChannels)
				{
	...
	public class ChannelList : CollectionBase, IList<ElfChannel>, ICollection<ElfChannel>
	{
		public ChannelList()
		{ }

		public ChannelList(ElfChannel item)
			: this()
		{
			List.Add(item);
		}

		public void Add(ElfChannel item)
		{
			List.Add(item);
		}
		...
		public void AddRange(ElfChannel[] collection)
		{
			for (int i = 0; i < collection.Length; i++)
				Add(collection[i]);
		}

		public bool Contains(ElfChannel item)
		{
			return List.Contains(item);
		}

		public void CopyTo(ElfChannel[] array, int arrayIndex)
		{
			List.CopyTo(array, arrayIndex);
		}

		public int IndexOf(ElfChannel item)
		{
			return List.IndexOf(item);
		}

		public void Insert(int index, ElfChannel item)
		{
		...
		public ChannelList OrderByDescending()
		{
			...
			ElfChannel[] Arr = ToArray();
			Array.Sort(Arr);
			Stack<ElfChannel> Stack = new Stack<ElfChannel>();
		...
		public void Remove(ElfChannel item)
		{
			if (List.Contains(item))
				List.Remove(item);
		}

		bool ICollection<ElfChannel>.Remove(ElfChannel item)
		{
			if (!List.Contains(item))
				return false;
			List.Remove(item);
		...
		{
			get { return (ElfChannel)List[index]; }
			set { List[index] = value; }
		}

		public ElfChannel[] ToArray()
		{
			ElfChannel[] Arr = new ElfChannel[Count];
			for (int i = 0; i < Count; i++)
				Arr[i] = this[i];
			return Arr;
		}

		public ElfChannel Where(int id)
		{
			foreach (ElfChannel item in List)
				if (item.ID == id)
					return item;
			return null;
		}
		...
		private class ChannelListEnumerator : IEnumerator<ElfChannel>
		{
			...
			public ElfChannel Current
			{
				get { return (ElfChannel)_enumerator.Current; }
			}
