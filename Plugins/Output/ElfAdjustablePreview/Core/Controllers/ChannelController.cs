...
using ElfAdjustablePreview.Core.Core;
using ElfAdjustablePreview.Core.Profiles;
using ElfAdjustablePreview.Core.Util;

namespace ElfAdjustablePreview.Core.Controllers
{
	/// <summary>
	/// Controls the list of Channels and maintains which Channels are active, grouped, etc.
	/// </summary>
	public class ChannelController : ElfBase, IEnumerable<ElfChannel>
	{
		...
		/// </summary>
		private ElfChannel _active = null;
		...
		public bool CanMoveDown
		{
			get
			{
				...
				{
					Found = false;
					foreach (ElfChannel Channel in Selected)
					{
						if (Channel.Index == i)
						{
							Found = true;
		...
		public int MaxID
		{
			...
			{
				int MaxID = -1;
				foreach (ElfChannel Channel in _channels)
		...
		[DebuggerHidden()]
		public ElfChannel this[int index]
		{
			get
			{
				if (_sortedChannels.Count == 0)
		...
		public void Add(ElfChannel channel)
		{
			...
			ElfChannel Ch = _channels.Where(channel.ID);
		...
		/// <exception cref="System.ArgumentNullException">Channel is null</exception>
		public void Add(ElfRawChannel rawChannel)
		{
			// If the channel ID is missing or is already being used by an existing channel, optain a new one.
			ElfChannel Ch = _channels.Where(rawChannel.ID);
		...
		public void AddRange(List<ElfChannel> channels)
		{
			if (channels == null)
				throw new ArgumentNullException("collection is null");

			foreach (ElfChannel Channel in channels)
				Add(Channel);
		}
		...
		/// <exception cref="System.ArgumentNullException">Channel is null</exception>
		public bool Contains(ElfChannel channel)
		{
			if (channel == null)
		...
		public int IndexOf(ElfChannel channel)
		{
			if (channel == null)
				throw new ArgumentNullException("channel is null");
		...
		/// <exception cref="System.ArgumentNullException">Channel is null</exception>
		public virtual void Insert(int index, ElfChannel channel)
		{
		...
		public void Remove(ElfChannel channel)
		{
			if (channel == null)
				throw new ArgumentNullException("channel is null");
			...
			foreach (ElfChannel Channel in _channels.OrderByAscending())
			{
				if (Channel.ID > ID)
					Channel.ID--;
			}
		...
		IEnumerator<ElfChannel> IEnumerable<ElfChannel>.GetEnumerator()
		{
			return (IEnumerator<ElfChannel>)GetEnumerator();
		}
		...
		/// <exception cref="System.ArgumentNullException">Channel is null</exception>
		private void AttachChildEvents(ElfChannel channel, bool attachEvents)
		{
			if (channel == null)
				throw new ArgumentNullException("channel is null");
		...
		public void CopyFrom(ChannelController source)
		{
			...
			foreach (ElfChannel Channel in source.GetAllChannels())
				Add((ElfChannel)Channel.Clone());
		...
		public void EmptyChannels(ChannelList collection)
		{
			...
			foreach (ElfChannel Channel in collection)
				Channel.Empty();
		...
		public void SetRenderColor(ChannelList collection, Color color)
		{
			...
			SuppressChannelEvents(collection, true);
			foreach (ElfChannel c in collection)
				c.RenderColor = color;
			SuppressChannelEvents(collection, false);

			OnPropertyChanged(collection, ElfChannel.Property_RenderColor);
		}
		...
		public void SetSequencerColor(ChannelList collection, Color color)
		{
			...
			OnPropertyChanged(collection, ElfChannel.Property_SequencerColor);
		}
		...
		public void SetBorderColor(ChannelList collection, Color color)
		{
			...
			OnPropertyChanged(collection, ElfChannel.Property_BorderColor);
		}
		...
		public void SetVisible(ChannelList collection, bool visible)
		{
			...
			OnPropertyChanged(collection, ElfChannel.Property_Visible);
		}
		...
		public void UnselectAll(bool fireEvent)
		{
			...
			foreach (ElfChannel Channel in Unselected)
		...
		public void PopulateChannelFromBitmap(Bitmap bitmap, ElfChannel channel, bool clearFirst)
		{
			if (bitmap == null)
		...
		/// <exception cref="System.ArgumentNullException">Neither the bitmap, nor the Channel can be null.</exception>
		public void PopulateChannelFromBitmap(Bitmap bitmap, ElfChannel channel)
		{
		...
		/// <param name="channel">Channel to modify</param>
		/// <exception cref="System.ArgumentNullException">Neither the bitmap, nor the Channel can be null.</exception>
		public void PopulateChannelFromBitmap_Erase(Bitmap bitmap, ElfChannel channel)
		{
			if (bitmap == null)
				throw new ArgumentNullException("bitmap cannot be null.");
			if (channel == null)
		...
		private void OnChannelAdded(ElfChannel channel)
		{
			if (SuppressEvents)
				return;
		...
		private void OnChannelsChanged(ElfChannel channel, string propertyName)
		{
			if (SuppressEvents)
				return;
		...
		private void OnChannelRemoved(ElfChannel channel)
		{
			if (SuppressEvents)
				return;
		...
		private void OnChannelsSelected(ElfChannel channel)
		{
			if (SuppressEvents)
				return;
		...
		{
			if (e.PropertyName != ElfChannel.Property_Selected)
				OnPropertyChanged(sender, e.PropertyName);
			else
				OnChannelsSelected((ElfChannel)sender);
		}
