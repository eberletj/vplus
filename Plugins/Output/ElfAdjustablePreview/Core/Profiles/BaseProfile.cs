...
namespace ElfAdjustablePreview.Core.Profiles
{
	public sealed class BaseProfile : ElfBase
	{
		...
		{
			foreach (ElfRawChannel rChannel in _profileDataLayer.Channels)
		...
		public bool Load(XmlNode node, List<ElfRawChannel> rawChannelList)
		{
			SuppressEvents = true;
		...
		public bool Save()
		{
			...
			try
			{
				...
				{
					_profileDataLayer.Channels.Add(new ElfRawChannel()
					{
		...
		{
			if (sender is ElfChannel)
			{
				// Trigger the ChannelPropertyChanged event with a new array of 1 Channel.
				OnChannelPropertyChanged(new ChannelList((ElfChannel)sender), e.PropertyName);
			}
